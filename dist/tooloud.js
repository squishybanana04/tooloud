		...
		*/
		...
		module.exports = {
			...
			Worley: {
				...
				create: function(seed) {
					return new Worley(seed)
				}
			},

			Fractal: {
				Classic: fractal.Classic,
				Billow: fractal.Billow,
				RigidMulti: fractal.RigidMulti,
				create: function() {
					return new Fractal()
				}
			}
	...
	function(module, exports) {
		...
		function Worley(seed) {
			...
			function EuclideanDistance(p1, p2) {
				return d(p1, p2).reduce(function(sum, x) {
					return sum + (x * x)
				}, 0);
			}
			...
			function noise(input, distanceFunc) {
				...
				return distanceArray.map(function(x) {
		...
		function Fractal() {
			...
			function fractalRigidMulti(x, y, z, octaves, noiseCallback) {
				...
				for (var i = 1; i < octaves; i++) {
					t += 1 / f;
					f *= 2;
					sum -= (1 - Math.abs(noiseCallback(x * f, y * f, z * f))) / f
				}
				return sum / t;
			}
			return {
